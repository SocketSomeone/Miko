...
for (const [packageName, packagePath] of packagePathMap.entries()) {
	...
	const tsconfigData = {
		extends: pathJoin(pathRelative(packagePath, rootPath), ConfigFilename.BASE_TSCONFIG),
		compilerOptions: {
			outDir: './lib',
			rootDir: './src',
			composite: true
		...
		references: internalDependencies.map(dependencyName => {
			...
			return {
				path: pathJoin(pathRelative(packagePath, dependencyPath), ConfigFilename.PACKAGE_TSCONFIG)
			};
		})
	};
...
const tsconfigProjectData = {
	...
	references: resolveInternalDependencies([...packagePathMap.keys()]).map(dependencyName => {
		...
		return {
			path: pathJoin('..', pathRelative(rootPath, dependencyPath), ConfigFilename.PACKAGE_TSCONFIG)
		};
	})
};
